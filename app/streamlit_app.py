...
def _render_input_panel() -> None:
    """Render the CSV/IFC input panel shared by all pages."""
    ...
    with st.sidebar:
        ...
        input_type = st.radio(
            ...
            index=0,
            key="input-type",
            help="CSV 是默认输入；IFC 需要单独安装可选 IfcOpenShell。",
        )
        if input_type == "IFC":
            elements_upload = st.file_uploader(
                ...
                type=["ifc"],
                key="ifc-upload",
                help="IFC reader 为可选功能；缺少依赖时 CSV 分支仍可用。",
            )
        else:
            elements_upload = st.file_uploader(
                "构件明细 CSV",
        ...
        if st.button("加载数据", key="load-data"):
            if elements_upload is None:
                label = "IFC 模型" if input_type == "IFC" else "构件明细 CSV"
                st.session_state["pipeline_error"] = f"请先选择{label}。"
                ...
                try:
                    ...
                    if input_type == "IFC":
                        st.session_state["artifacts"] = load_artifacts_from_ifc_bytes(
                    ...
                    st.session_state["pipeline_error"] = (
                        f"数据加载失败：{exc}；请检查 {input_type} 文件和必需列后重试。"
                    )

        if st.session_state.get("pipeline_error"):
            st.error(st.session_state["pipeline_error"])
        elif st.session_state.get("artifacts") is not None:
            st.success(f"{input_type} 数据已加载，可在上方页面导航中查看结果。")
